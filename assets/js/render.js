...
window.RSPBA_RENDER = (function () {
  ...
  // --- 1. HOMEPAGE FLOATING EVENT CARD (with auto-advancing countdown)
    ...
    el.innerHTML = list.map(evt => {
      ...
      const ctaHref = isPast
        ...
        : isNext
          ...
          : 'diary.html#' + evt.id;
      const ctaLabel = isPast ? 'Results →' : isNext ? 'Enter →' : 'Details →';
      return `
        <article class="${cls}">
          <header>
  ...
  function homeNews(el) {
    ...
    el.innerHTML = news.map((item, idx) => {
      ...
      const placeholder = isLg ? 'Contest action · 16:9' : 'Image · 4:3';
      return `
        <article class="card ${isLg ? 'card-lg' : ''}">
          <div class="image-slot rounded" role="img" aria-label="${escapeAttr(placeholder)}">${escapeHtml(placeholder)}</div>
          <div class="card-body">
            <div class="card-meta">
              <span class="card-tag ${tagClass}">${escapeHtml(item.tag || 'Notice')}</span>
              <time datetime="${escapeAttr(item.date)}">${RSPBA.formatDate(item.date, 'long')}</time>
            </div>
  ...
  function contestList(el) {
    ...
    el.innerHTML = events.map(evt => {
      ...
      const cta = isPast
        ? `<a class="btn btn-text" href="results.html${evt.resultsAnchor ? '#' + evt.resultsAnchor : ''}">Results →</a>`
        : isNext
          ? `<a class="btn btn-primary" href="${escapeAttr(evt.entryForm || 'placeholder.html?doc=' + encodeURIComponent(evt.name + ' entry form'))}">Enter contest →</a>
  ...
  function diaryTimeline(el) {
    ...
    el.innerHTML = events.map(evt => {
      ...
      const ctaLabel = isPast ? 'Results →' : isNext ? 'Enter →' : 'Entry forms →';
      return `
    ...
    el.innerHTML = news.map(item => {
      ...
      return `
        <li>
          ...
          <a class="nf-cta" href="${escapeAttr((item.link || {}).url || '#')}">${escapeHtml((item.link || {}).label || 'Read')} →</a>
        </li>
      `;
    }).join('');
  }
